...
In 2020, when my X1's RAM module failed after only months of use, requiring complete replacement, I elected to replace Lenovo instead.
...
```default
# systemd-cryptenroll --tpm2-device=auto --tpm2-pcrs="0+6+7+15" /dev/nvme0n1p2
```

PCR 15 with filesystem verification is especially important: without it attackers may [bypass disk encryption](https://oddlama.org/blog/bypassing-disk-encryption-with-tpm2-unlock/). Most of the guides online---and my first pass at this configuration---miss this key detail. You will need to tell the system to measure the filesystem into PCR 15. I do this via a kernel boot param:

```default
rd.luks.options=${LUKS_UUID}=tpm2-device=auto,tpm2-measure-pcr=yes
```

The first boot will require a password. Afterwards, re-enrollment with the finalized value of PCR 15 (simply repeating the `systemd-cryptenroll` command) will cause auto-unlock to fail if the encrypted volume has been tampered with.
