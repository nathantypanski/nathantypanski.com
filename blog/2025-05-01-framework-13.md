...
I use PCRs 0, 4, 6, and 7 to measure `platform-code`, `boot-loader-code`, `host-platform`, and `secure-boot-policy`. This aggressive configuration (most people only use PCR 7) ensures the TPM only releases keys when the entire Secure Boot chain is untampered. The TPM integration means automatic disk decryption on clean boots, with fallback to password entry if the measured boot state changes.
